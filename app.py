...
h1, h2, h3 { font-family: 'IBM Plex Mono', monospace !important; color: #e6edf3 !important; letter-spacing: -0.02em; }
.stat-card { background: #161b22; border: 1px solid #21262d; border-radius: 8px; padding: 16px 20px; text-align: center; }
.stat-value { font-family: 'IBM Plex Mono', monospace; font-size: 28px; font-weight: 600; color: #58a6ff; line-height: 1.1; }
.stat-label { font-size: 11px; color: #8b949e; text-transform: uppercase; letter-spacing: 0.08em; margin-top: 4px; }
.stat-sub { font-family: 'IBM Plex Mono', monospace; font-size: 12px; color: #3fb950; margin-top: 2px; }
.section-header { font-family: 'IBM Plex Mono', monospace; font-size: 11px; color: #8b949e; text-transform: uppercase; letter-spacing: 0.1em; border-bottom: 1px solid #21262d; padding-bottom: 8px; margin-bottom: 16px; }
div[data-testid="stSelectbox"] label, div[data-testid="stMultiSelect"] label, div[data-testid="stSlider"] label { color: #8b949e !important; font-size: 12px !important; text-transform: uppercase; letter-spacing: 0.06em; }
.stButton > button { background: #1f3a5f; color: #58a6ff; border: 1px solid #1f6feb; border-radius: 6px; font-family: 'IBM Plex Mono', monospace; font-size: 13px; }
.stButton > button:hover { background: #1f6feb; color: #ffffff; }
div[data-testid="stMetricValue"] { font-family: 'IBM Plex Mono', monospace !important; color: #58a6ff !important; }
.stTabs [data-baseweb="tab-list"] { background-color: #161b22; border-bottom: 1px solid #21262d; }
.stTabs [data-baseweb="tab"] { font-family: 'IBM Plex Mono', monospace; font-size: 13px; color: #8b949e; }
...
def load_batting(season: int, min_pa: int) -> pd.DataFrame:
    ...
    try:
        ...
        local = os.path.join(os.path.dirname(__file__), "data_batting.csv")
...
def load_pitching(season: int, min_ip: int) -> pd.DataFrame:
    ...
    try:
        ...
        local = os.path.join(os.path.dirname(__file__), "data_pitching.csv")
        df = pd.read_csv(local)
        df = df[df["IP"] >= min_ip]
    return df.reset_index(drop=True)
...
with st.sidebar:
    ...
    st.markdown("---")
    mode = st.radio("Dashboard mode", ["Batting","Pitching","Player Comparison"], index=0)
    st.markdown("---")
    ...
    st.markdown("---")
    st.markdown('<div style="font-size:11px;color:#8b949e;">Data via pybaseball · Baseball Reference & FanGraphs</div>', unsafe_allow_html=True)
...
if mode == "Batting":
    ...
    for col, label, val, player in [
        ...
        with col:
            st.markdown(f'<div class="stat-card"><div class="stat-value">{val}</div><div class="stat-label">{label}</div><div class="stat-sub">{player}</div></div>', unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    tab1,tab2,tab3,tab4 = st.tabs(["Scatter explorer","Top 20 leaderboard","Team breakdown","K% vs BB%"])

    with tab1:
        st.markdown('<div class="section-header">Interactive scatter</div>', unsafe_allow_html=True)
        cx,cy = st.columns(2)
        metrics = ["AVG","OBP","SLG","OPS","HR","RBI","BB%_num","K%_num","wRC+","WAR"]
        x_axis = cx.selectbox("X axis", metrics, index=9)
        y_axis = cy.selectbox("Y axis", metrics, index=8)
        fig = px.scatter(df, x=x_axis, y=y_axis, color="Team", hover_name="Name",
                         hover_data={"AVG":":.3f","HR":True,"OPS":":.3f","WAR":":.1f"})
        fig.update_layout(**PLOT_THEME, height=480, showlegend=False, margin=dict(l=0,r=0,t=20,b=0))
        fig.update_traces(marker=dict(opacity=0.75, size=9))
    ...
    with tab2:
        ...
        fig = px.bar(top20.sort_values("wRC+"), x="wRC+", y="Name", orientation="h",
                     color="wRC+", color_continuous_scale=["#1f3a5f","#58a6ff","#cae8ff"],
                     hover_data={"AVG":":.3f","HR":True,"WAR":":.1f"})
        fig.update_layout(**PLOT_THEME, height=540, coloraxis_showscale=False, margin=dict(l=0,r=0,t=10,b=0))
        st.plotly_chart(fig, use_container_width=True)
        st.dataframe(top20.reset_index(drop=True), use_container_width=True, hide_index=True,
    ...
    with tab3:
        ...
        team_stats = df.groupby("Team").agg(AVG=("AVG","mean"),OPS=("OPS","mean"),
                                             HR=("HR","sum"),wRC_plus=("wRC+","mean"),
                                             Players=("Name","count")).round(3).sort_values("OPS",ascending=False).reset_index()
        fig = make_subplots(rows=1, cols=2, subplot_titles=["Team OPS","Team HR total"])
        fig.add_trace(go.Bar(x=team_stats["Team"], y=team_stats["OPS"],  marker_color="#58a6ff", name="OPS"), row=1,col=1)
        fig.add_trace(go.Bar(x=team_stats["Team"], y=team_stats["HR"],   marker_color="#3fb950", name="HR"),  row=1,col=2)
        fig.update_layout(**PLOT_THEME, height=400, showlegend=False, margin=dict(l=0,r=0,t=40,b=0))
        fig.update_xaxes(tickangle=-45)
        st.plotly_chart(fig, use_container_width=True)

    with tab4:
        st.markdown('<div class="section-header">Plate discipline — K% vs BB%</div>', unsafe_allow_html=True)
        fig = px.scatter(df, x="BB%_num", y="K%_num", color="wRC+",
                         color_continuous_scale=["#1f3a5f","#58a6ff","#cae8ff"],
                         hover_name="Name", hover_data={"AVG":":.3f","HR":True,"wRC+":True},
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        st.markdown('<p style="font-size:12px;color:#8b949e">Above the line = outperforming FIP (lucky). Below = underperforming (unlucky).</p>', unsafe_allow_html=True)
        fig = px.scatter(df, x="FIP", y="ERA", color="WAR",
                         color_continuous_scale=["#f0883e","#8b949e","#3fb950"],
                         hover_name="Name", hover_data={"IP":":.0f","K/9":":.2f","WHIP":":.2f","WAR":":.1f"})
        mn = min(df["FIP"].min(), df["ERA"].min()) - 0.2
        mx = max(df["FIP"].max(), df["ERA"].max()) + 0.2
        fig.add_shape(type="line", x0=mn,y0=mn,x1=mx,y1=mx, line=dict(color="#8b949e",dash="dot",width=1))
        fig.update_layout(**PLOT_THEME, height=460, margin=dict(l=0,r=0,t=20,b=0))
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab2:
        ...
        fig = px.bar(top20.sort_values("WAR"), x="WAR", y="Name", orientation="h",
                     color="ERA", color_continuous_scale=["#3fb950","#58a6ff","#f0883e"],
                     hover_data={c:True for c in ["ERA","FIP","K/9"] if c in top20.columns})
        fig.update_layout(**PLOT_THEME, height=540, margin=dict(l=0,r=0,t=10,b=0))
        st.plotly_chart(fig, use_container_width=True)
        fmt_cfg = {c:st.column_config.NumberColumn(format="%.2f") for c in ["ERA","FIP","xFIP","WHIP","K/9","BB/9","WAR","IP"]}
        st.dataframe(top20.reset_index(drop=True), use_container_width=True, hide_index=True, column_config=fmt_cfg)

    with tab3:
        st.markdown('<div class="section-header">Pitcher plate discipline</div>', unsafe_allow_html=True)
        fig = px.scatter(df, x="BB%_num", y="K%_num", color="ERA",
                         color_continuous_scale=["#3fb950","#8b949e","#f0883e"],
                         hover_name="Name", hover_data={"ERA":":.2f","FIP":":.2f","WAR":":.1f"},
                         labels={"BB%_num":"BB%","K%_num":"K%"})
        fig.add_hline(y=df["K%_num"].mean(),  line_dash="dot", line_color="#8b949e", annotation_text="Avg K%")
        fig.add_vline(x=df["BB%_num"].mean(), line_dash="dot", line_color="#8b949e", annotation_text="Avg BB%")
        fig.update_layout(**PLOT_THEME, height=460, margin=dict(l=0,r=0,t=20,b=0))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if comp_type == "Batters":
        with st.spinner("Loading batting data…"):
            df = load_batting(year, min_pa)
        players = st.multiselect("Select players (2–6)", sorted(df["Name"].tolist()),
                                  default=df.nlargest(4,"WAR")["Name"].tolist()[:4], max_selections=6)
        if len(players) < 2:
        ...
        st.plotly_chart(fig, use_container_width=True)
        display_cols = [c for c in ["Name","Team","PA","AVG","OBP","SLG","OPS","HR","RBI","wRC+","WAR"] if c in sel.columns]
        st.dataframe(sel[display_cols].reset_index(drop=True), use_container_width=True, hide_index=True,
                     column_config={"AVG":st.column_config.NumberColumn(format="%.3f"),
                                    "OBP":st.column_config.NumberColumn(format="%.3f"),
                                    "SLG":st.column_config.NumberColumn(format="%.3f"),
                                    "OPS":st.column_config.NumberColumn(format="%.3f"),
        ...
        with st.spinner("Loading pitching data…"):
            df = load_pitching(year, min_ip)
        players = st.multiselect("Select pitchers (2–6)", sorted(df["Name"].tolist()),
                                  default=df.nlargest(4,"WAR")["Name"].tolist()[:4], max_selections=6)
        if len(players) < 2:
        ...
        st.dataframe(sel[pcols].reset_index(drop=True), use_container_width=True, hide_index=True,
                     column_config={c:st.column_config.NumberColumn(format="%.2f") for c in ["ERA","FIP","xFIP","WHIP","K/9","BB/9","WAR","IP"]})
